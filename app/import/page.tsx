        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [
              ...
              { type: "text", text: `この画像はSIKKEN GROUPの月次営業データの表です。
...
- 合計列は除外する` },
      ...
      if (!jsonMatch) throw new Error("データの抽出に失敗しました。もう一度試してください。");
      const parsed = JSON.parse(jsonMatch[0]);
      const rows: ExtractedRow[] = (parsed.rows ?? []).map((r: ExtractedRow) => ({
        ...r,
