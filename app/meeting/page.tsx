        ...
        {activeBusiness === "water" && (
          <>
            ...
            <div className="metrics-grid-2col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              ...
              {/* PR c94-A: 旬独立化により部門別実績テーブル (depts: self/newSales/help) を全 period で非表示。
                  depts は calculateDashboard が legacy DailyEntry フィールド (selfRevenue/newRevenue/addRevenue)
                  を集計するが c90 以降 entries.data に未書込のため事実上機能していない (c93-5b と同根)。
                  削除はせず非表示のみ、c94-B (WaterMeetingSection 化) で monthly_summary 同形に再設計予定。 */}
              {false && (
