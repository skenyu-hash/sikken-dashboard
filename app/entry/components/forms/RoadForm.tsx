...
export default function RoadForm({ state, setField, validateField, errors, labels, calc, vehicleSnapshot, traineeSnapshot }: Props) {
  ...
  return (
    <>
      ...
      <SectionShell title={labels.section_sales} subtitle="入力 6項目 (販管費は記録のみ) + 自動計算 (売上比 / 粗利)" group="rev" count={9} defaultOpen>
      ...
      <SectionShell title="② 入電" subtitle="入力 7項目 + 自動計算 (総入電件数 / 入電単価)" group="acq" count={9}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 12 }}>
      ...
      <SectionShell title="③ 獲得" subtitle="入力 7項目 + 自動計算 (総獲得件数 / 獲得単価 / 成約率)" group="acq" count={10}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 12 }}>
          {CHANNEL_KEYS.map((key) => (
