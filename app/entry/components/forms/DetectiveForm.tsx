...
export default function DetectiveForm({ state, setField, validateField, errors, labels, calc }: Props) {
  ...
  return (
    <>
      {/* ① 新規対応セクション */}
      <SectionShell title={labels.section_sales} subtitle="入力 3項目 (販管費は記録のみ) + 自動計算 (営業利益)" group="rev" count={4} defaultOpen>
      ...
      <SectionShell title="② 入電" subtitle="入力 4項目 + 自動計算 (入電数 / 入電単価)" group="acq" count={6}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 12 }}>
      ...
      <SectionShell title="③ 獲得" subtitle="入力 6項目 (3 媒体 × 2 カテゴリ) + 自動計算 (合計獲得件数 / 獲得単価)" group="acq" count={8}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 12 }}>
      ...
      <SectionShell title="④ 面談プロセス" subtitle="入力 3項目 + 自動計算 (アポ獲得率 / 面談率 / 成約率)" group="acq" count={7}>
