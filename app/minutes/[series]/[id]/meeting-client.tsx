// app/minutes/[series]/[id]/meeting-client.tsx
'use client';
...
type InitialData = {
  session: AnyRow;
  agendas: (AnyRow & { discussions: AnyRow[]; concerns: AnyRow[]; decisions: AnyRow[] })[];
  metrics: AnyRow | null;
};
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '24px 20px' }}>
      <div style={{ background: PURPLE, color: '#fff', padding: '16px 24px', borderRadius: '12px 12px 0 0' }}>
        <a href={`/minutes/${seriesCode}`} style={{ display: 'inline-block', color: '#fff', opacity: 0.75, textDecoration: 'none', fontSize: 12, marginBottom: 6 }}>← 議事録一覧へ戻る</a>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16 }}>
          <div>
            <h1 style={{ fontSize: 20, fontWeight: 500, margin: 0 }}>{session.series_name} — {new Date(session.meeting_date).toLocaleDateString('ja-JP')}</h1>
            <p style={{ fontSize: 12, opacity: 0.85, margin: '4px 0 0' }}>
              {session.cycle_year}年{session.cycle_month}月{getCyclePeriodShortLabel(session.cycle_period)}
      ...
      <div style={{ background: '#fff', border: '0.5px solid rgba(0,0,0,0.1)', borderTop: 'none', borderRadius: '0 0 12px 12px', padding: '20px 24px' }}>
        ...
        {initial.agendas.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px 20px', color: '#888', border: '0.5px solid rgba(0,0,0,0.1)', borderRadius: 8, background: '#FAFAFA', fontSize: 13 }}>議題がまだありません。「+ 議題を追加」から始めてください。</div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {initial.agendas.map((a) => <AgendaCard key={a.id} agenda={a} onChange={refresh} />)}
          </div>
        )}
      </div>
    </div>
...
function StatusChip({ status }: { status: string }) {
  const map: Record<string, { label: string }> = { scheduled: { label: '予定' }, in_progress: { label: '進行中' }, completed: { label: '完了' } };
  const s = map[status] ?? map.in_progress;
  return <span style={{ background: 'rgba(255,255,255,0.2)', color: '#fff', padding: '4px 10px', borderRadius: 6, fontSize: 11 }}>{s.label}</span>;
}
  ...
  if (!metrics) {
    return (
      <div style={{ background: PURPLE_LIGHT, padding: '12px 20px', borderBottom: '0.5px solid rgba(0,0,0,0.08)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 12, color: PURPLE_DARK }}>
        ...
        <button onClick={sync} disabled={loading} style={{ background: '#fff', color: PURPLE, border: `0.5px solid ${PURPLE_MID}`, padding: '4px 12px', borderRadius: 6, fontSize: 11, cursor: 'pointer' }}>{loading ? '取り込み中…' : '数字を取り込む'}</button>
      </div>
    );
  }
...
function Stat({ label, value }: { label: string; value: string }) {
  return (<div><div style={{ fontSize: 11, color: PURPLE_MID }}>{label}</div><div style={{ fontSize: 16, fontWeight: 500, color: PURPLE_DARK }}>{value}</div></div>);
}
    ...
    <div style={{ border: '0.5px solid rgba(0,0,0,0.1)', borderRadius: 8, overflow: 'hidden' }}>
      ...
      <div style={{ padding: '14px' }}>
        <Section dotColor={BLUE} title="発言・意見" count={agenda.discussions.length}>
          {agenda.discussions.length === 0 ? <Hint>発言を記録してください</Hint> : (
            <div style={{ marginBottom: 8 }}>
              {agenda.discussions.map((d) => (
                <div key={d.id} style={{ fontSize: 13, lineHeight: 1.7 }}><span style={{ color: BLUE, fontWeight: 500 }}>{d.speaker_name}：</span><span style={{ color: '#222' }}>{d.content}</span></div>
              ))}
            </div>
          )}
          <SpeakerInputForm onSubmit={async (sp, c) => { await fetch(`/api/agendas/${agenda.id}/discussions`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ speaker_name: sp, content: c }) }); onChange(); }} placeholder="発言内容" />
        </Section>
        <Section dotColor={AMBER} title="補足事項・懸念事項" count={agenda.concerns.length}>
          {agenda.concerns.length === 0 ? <Hint>補足や懸念があれば記録してください</Hint> : (
            <div style={{ marginBottom: 8 }}>
              {agenda.concerns.map((c) => (
                <div key={c.id} style={{ background: AMBER_LIGHT, borderLeft: `3px solid ${AMBER}`, borderRadius: '0 4px 4px 0', padding: '6px 10px', marginBottom: 4, fontSize: 13, lineHeight: 1.6 }}><span style={{ color: AMBER_DARK, fontWeight: 500 }}>{c.speaker_name}：</span><span style={{ color: AMBER_TEXT }}>{c.content}</span></div>
              ))}
            </div>
          )}
          <SpeakerInputForm onSubmit={async (sp, c) => { await fetch(`/api/agendas/${agenda.id}/concerns`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ speaker_name: sp, content: c }) }); onChange(); }} placeholder="補足や懸念事項" />
        </Section>
        <Section dotColor={TEAL} title="決定事項" count={agenda.decisions.length}>
          {agenda.decisions.length === 0 ? <Hint>まだ決定事項がありません</Hint> : (
            <div style={{ marginBottom: 8 }}>
              {agenda.decisions.map((d) => (
                <div key={d.id} style={{ background: TEAL_LIGHT, borderRadius: 4, padding: '6px 10px', marginBottom: 4, fontSize: 13, lineHeight: 1.6, color: TEAL_DARK }}><span style={{ color: TEAL, fontWeight: 500 }}>✓</span><span style={{ marginLeft: 6 }}>{d.content}</span></div>
              ))}
            </div>
          )}
          <SimpleInputForm onSubmit={async (c) => { await fetch(`/api/agendas/${agenda.id}/decisions`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content: c }) }); onChange(); }} placeholder="決定事項" />
        </Section>
...
function AddAgendaForm({ sessionId, onAdded }: { sessionId: number; onAdded: () => void }) {
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try { await fetch(`/api/meetings/${sessionId}/agendas`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: title.trim() }) }); setTitle(''); setOpen(false); onAdded(); } finally { setBusy(false); }
  };
  if (!open) return <button onClick={() => setOpen(true)} style={{ background: PURPLE, color: '#fff', border: 'none', padding: '6px 14px', borderRadius: 6, fontSize: 12, fontWeight: 500, cursor: 'pointer' }}>+ 議題を追加</button>;
  return (
...
function SpeakerInputForm({ onSubmit, placeholder }: { onSubmit: (speaker: string, content: string) => Promise<void>; placeholder: string }) {
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try { await onSubmit(speaker.trim(), content.trim()); setContent(''); } finally { setBusy(false); }
  };
  return (
...
function SimpleInputForm({ onSubmit, placeholder }: { onSubmit: (content: string) => Promise<void>; placeholder: string }) {
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try { await onSubmit(content.trim()); setContent(''); } finally { setBusy(false); }
  };
  return (
    <div style={{ display: 'flex', gap: 6, marginTop: 4 }}>
      <input value={content} onChange={(e) => setContent(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && submit()} placeholder={placeholder} style={{ flex: 1, padding: '6px 10px', border: '0.5px solid rgba(0,0,0,0.2)', borderRadius: 6, fontSize: 13 }} />
...
function NotesEditor({ agendaId, initialNotes }: { agendaId: number; initialNotes: string | null }) {
  ...
  const save = async (val: string) => {
    ...
    try { await fetch(`/api/agendas/${agendaId}/notes`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ notes: val }) }); setSaved(true); setTimeout(() => setSaved(false), 1500); } finally { setSaving(false); }
  };
  ...
  return (
    <div>
      ...
      <div style={{ fontSize: 11, color: '#aaa', marginTop: 4, height: 14 }}>{saving ? '保存中…' : saved ? '✓ 保存しました' : ''}</div>
    </div>
  );
}
