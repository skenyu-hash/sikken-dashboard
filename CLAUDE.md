...
### HELP率を「件数浸透率」に再定義 + 旧売上比をHELP売上サブへ移設（2026-06-22）
- **PR #187** ✅ マージ済 (2026-06-22T08:57Z、squash commit a6055e4) — HELP率の定義を「HELP売上 ÷ 売上 × 100」から「HELP件数 ÷ 実働母数 × 100（浸透率）」に変更。旧定義（売上比）はダッシュボードの「HELP売上」行に `売上比 XX%` サブ表記として移設（①新規対応と同形式）。**現場46名が見るKPI定義変更**（§0「経営数字の定義を変える変更」「現場が見る数字が変わる変更」に該当）。
  - **定義（業態ごとに実働母数を揃える ＝ 客単価の分母と一致）**: water/electric = `HELP件数 ÷ 対応件数(total_count)`、locksmith = `HELP件数 ÷ 総獲得件数(acquisition_count)`。鍵は対応件数を入力せず総獲得件数(=5内訳の和、HELPは5番目の構成要素)を実働母数にするため。鍵HELP率は「構成比」(HELP⊆総獲得)、水道/電気は「対応ボリュームに対する浸透率」（help_countは独立カウンタで対応件数に必ず含まれるとは限らない）。
  - **変更ファイル（6つ、表示専用）**: `{Water,Electric,Locksmith}DashboardSection.tsx`（helpRate式 + HELP売上の売上比サブ + HELP率サブ注記 + 前月同日比mom分母）/ `{Water,Electric,Locksmith}MeetingSection.tsx`（式のみ統一、MetricRowにsub機構なし）。入力フォームの自動計算HELP率は旧式据え置き（スコープ外）。**DB/aggregation/粗利・売上の計算は無変更**。
  - **目標値**: targetHelpRate は残置。新定義で現場が再設定する前提（反さん承認済）。旧目標（売上比前提、例8.0%）のままだと達成率が噛み合わないため現場で入れ直しが必要。
