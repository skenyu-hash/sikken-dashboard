// PR c95-B-2 → c95-D-4 統合テスト: water 業態の aggregation に手入力 consultant_fee
//   が正しく組み込まれ、他業態には影響しないことを検証 (DB 要、専用月 2099-12)。
//
// c95-D-6 (slice 6) でテスト内容を新仕様 (手入力ベース) に書き換え:
//   - 旧 c95-B-2: water + 5月以降 → revenue × 0.077 を自動控除
//   - 新 c95-D-4: water + 5月以降 → entries.data.consultant_fee を SUM 直接控除
//
// 実行: export $(grep DATABASE_URL .env.local | xargs) && npm run test:integration:c95-b-aggregation-consultant
//
// 検証範囲:
//   1. water + yyyymm=209912 (>=202605) + consultant_fee=77000 → revenue - costs - 77000
//   2. electric + 同月 + consultant_fee=77000 → revenue - costs (controle 経路に入らない、water 限定)
//   3. road + 同月 → 同上 (controle なし)
//   4. detective + 同月 → 同上
//   5. locksmith + 同月 → locksmith 専用式 (元から ELSE 分岐外、影響なし regression)
//   6. water + 売上 0 + consultant_fee=0 → 0 (異常値ガード)
//
// 注: 月境界 (yyyymm < 202605) ケースは aggregation SQL の 3 段 CASE 内で
//   sum_consultant_fee 完全除外分岐 (slice 4 で追加) が発火するが、本テストは 2099-12 のみ。
//   4 月以前データ touch を完全回避するため、本テストは 2099-12 のみ使用。
...
async function main() {
  ...
  console.log(`🧪 PR c95-D-4: aggregation 手入力 consultant_fee (water 限定) DB 検証`);
  console.log(`   投入先: ${TEST_AREA}/${TEST_YEAR}-${TEST_MONTH} (yyyymm=${TEST_YEAR * 100 + TEST_MONTH} >= 202605、控除対象月)\n`);

  const pool = new Pool({ connectionString: process.env.DATABASE_URL });
  const client = await pool.connect();
  try {
    // ── 1. water 業態: 手入力 consultant_fee が直接控除される ──
    //   revenue = 1,000,000、costs = 200000+100000+50000+30000+5000 = 385000
    //   c93-1 式 (consultant_fee なし): 1000000 - 385000 = 615000
    //   c95-D-4 手入力 77,000 控除: 615000 - 77000 = 538000
    console.log("📋 1. water 業態: 手入力 consultant_fee=77000 直接控除 (538000)");
    await setupCellWithEntry(client, "water", {
      ...
      consultant_fee: 77000, // c95-D-4: 手入力ベース
    });
    ...
    //   同じ売上・コスト + consultant_fee=77000 でも water 限定 SQL CASE で控除されない
    console.log("\n📋 2. electric 業態: water 限定ガードで controle 0 (615000)");
    await setupCellWithEntry(client, "electric", {
      ...
      sales_outsourcing_cost: 30000,
      card_processing_fee: 5000,
      consultant_fee: 77000, // electric には影響しない (water 限定 SQL CASE)
    });
    await aggregateMonthlySummary(TEST_AREA, "electric", TEST_YEAR, TEST_MONTH);
    check("electric total_profit (= 615000、c93-1 式そのまま、consultant_fee 無視)", await readTotalProfit(client, "electric"), 615000);
    await cleanup(client, "electric");

    // ── 3. road 業態: 控除されない ──────────────────────
    console.log("\n📋 3. road 業態: water 限定ガードで controle 0 (615000)");
    ...
    console.log("\n📋 4. detective 業態: water 限定ガードで controle 0 (615000)");
    ...
    //   locksmith 式: 200000 - 50000 - 0 - 0 - 0 = 150000 (consultant_fee 無関係)
    ...
    await setupCellWithEntry(client, "locksmith", {
      ...
      consultant_fee: 77000, // locksmith 専用式に登場しない、無視される
    });
    await aggregateMonthlySummary(TEST_AREA, "locksmith", TEST_YEAR, TEST_MONTH);
    check("locksmith total_profit (= 150000、consultant_fee 無関係)", await readTotalProfit(client, "locksmith"), 150000);
    await cleanup(client, "locksmith");

    // ── 6. water + 売上 0 円 + consultant_fee=0 (異常値ガード regression) ─
    console.log("\n📋 6. water 売上 0 + consultant_fee=0 → total_profit = 0");
    await setupCellWithEntry(client, "water", {
    ...
    await aggregateMonthlySummary(TEST_AREA, "water", TEST_YEAR, TEST_MONTH);
    check("water 売上 0 + cf=0 → total_profit = 0", await readTotalProfit(client, "water"), 0);
    await cleanup(client, "water");
