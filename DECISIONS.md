...
## D-011: 会社別ビューは「案A（シンプル）」を採用 — 業態別セクション非表示 (PR #169)
- 日付: 2026-06-13
- 決めたこと: 会社別ビュー (`viewMode="company"`) では、業態別KPIセクション・KPIストリップ・達成率・CSV出力を非表示にする。表示するのは「ヒーロー（全社売上・粗利）」と「CompanyBreakdownTable（32行の事業×エリア内訳）」のみ。詳細は「事業別で編集→」ボタンで事業別ビューへ遷移して確認する設計。
- なぜ: DUNK社（水道×2 + ロード×1 + 電気×1）のように複数業態を持つ会社では、CPA・成約率・入電単価などの効率指標は業態別の分母・分子を合算してから再計算する必要があり、単純に平均できない。これをフロントエンドで全業態分実装・維持するのはバグの温床になる。Geminiによる独立アーキテクチャレビュー（2026-06-13）でも案Aを強く推奨。
- 却下した代替案:
  - **案B（複雑）**: 会社別ビューに業態別セクションを並列表示 → 上記の理由で却下。複数業態会社での合算再計算ロジックが必須、DUNKの3業態が縦に並ぶとUXも悪化
  - **案C（中間）**: 共通指標のみのサマリーカード → CompanyBreakdownTableに合計行を追加すれば案Aで代替できるため不採用
- 参照: Geminiレビュー結果は `docs/specs/AI_CONTEXT_COMPANY_VIEW_WITH_CODE.md` に格納
