...
## D-013: 年次(YTD)ビューの目標表示は「月次画面と同じ目標比」に統一（ペース比・年間目標は不採用）
- 日付: 2026-06-20（year-view スライス4、反専務 承認）
- 決めたこと: 年次ビューのヒーローは「**目標比 = 年初来累計(YTD)実績 ÷ 入力済み月の月次目標の合計**」だけを出す。月次画面の「目標比」と全く同じ計算の流儀で、それを5月から足し算しただけ。
- なぜ（反社長向けの言葉で）:
  1. **経営者がすでに毎日読めている指標に揃えるのが一番分かりやすい**。月次画面の「目標比 71.6%」をそのまま年次に拡張するだけなら、新しい覚え方が要らない。
  2. **目標は「月次目標管理（/targets）」で月ごとに手入力されたもの**が唯一の正。年間目標を1本で入れる仕組みは現時点で存在しない。だから年次の「目標」は“入力済みの月の合計”であって、画面にもそう正直に注記する（例: 今は5・6月分のみ）。月が進めば自動的に本物の年間実績に育つ。
  3. **「ペース比」（当月を日割り按分した達成率）は却下**。理屈は正しい（月初に達成率が暴落して見えるのを防ぐ）が、反専務が「よく分からない」と判断 → 経営者が一目で理解できない指標は採用しない、という基準を優先。理論的な正しさより現場の理解しやすさを取った。
- 却下した代替案:
  - 案B（年次は実績だけ、目標を出さない）→ せっかく入力されている月次目標を捨てるので不採用。
  - 案C（ペース比＋年間目標を維持）→ 上記2と3の理由で却下。
- 将来（別テーマ、未着手）: 期初に「**年間予算を1本入力する仕組み**」を作れば、本物の年間目標 vs 実績ができる。IPO/持株会社化に向けた予算管理の土台。新テーブル＋入力画面＋運用設計が要る新機能なので、BACKLOG に記録し、やると決めたら別途設計する。
- 関連: 集計の数値ロジック自体（粗利の損失消去封殺・率の総額再計算・4月以前ガード=YTD_MIN_YYYYMM 202605）は Gemini 独立レビューで設計し、番人 number-verifier / invariant-guard で検証済み（PR #179/#180/#181）。本決定はその「表示の見せ方」だけを月次に合わせる調整。

## D-012: 鍵 入電内訳は「現場エクセルを正」として過去47日を埋め戻し、ズレ8日は call_count を上書き
- 日付: 2026-06-18（PR #172 マージ後の運用作業）
- 決めたこと: 鍵業態（関西）の入電内訳（車LP+メール / インハウス）を、現場エクセル（5月・6月）を真実のソースとして本番 entries に埋め戻した。`scripts/backfill-locksmith-call-breakdown.ts`（dry-run デフォルト / `--apply`）で実行。エクセル内訳の和と DB 既存 call_count が食い違う 8 日（5/4・5/17・5/19・5/22・5/25・6/1・6/8・6/10）は、call_count をエクセル値で**上書き**（加算ではない）。
- なぜ: PR #172 以前は内訳が DB 保存されず（LocalNumberField で破棄）、合計 call_count のみ保存されていた。現場エクセルは車LP+メール／インハウスを分離記録しており、これが唯一の過去内訳ソース。車LP+メール（広告由来）とインハウス（無広告）の分離は戦略目標①「広告依存からの脱却」の計器であり、復元価値が高い。8 日のズレは「現場がエクセルを後修正 or DB 入力時の微差」と判断し、現場運用に近いエクセルを正とした（反専務後任の承認、2026-06-18）。
- 数値影響（経営数字）: 月次 call_count は 5月 1177→1172（−5）、6月 574→576（+2）。入電単価は 5月 8,362→8,397円、6月 9,373→9,340円。売上・粗利・広告費・獲得件数は不変。
- 安全策: dry-run で全日照合（一致39 / ズレ8 / DB欠落1）→ 反専務承認 → `--apply`。適用前後で 4月以前 monthly_summaries（count=122 / 売上 / 粗利 / 入電件数 / updated_at）の完全不変をコード照合。入電以外フィールドの不変も適用後に照合。再集計は kansai×locksmith×5月/6月の2行のみ。
- 対象外: 6/17 は DB に entry が無いため埋め戻し対象外（現場が /entry で通常入力 → PR #172 で内訳も保存される）。
- 却下した代替案:
  - エクセルを正とせず一致39日のみ埋め、ズレ8日は内訳空のまま据置 → 8日が永久に内訳欠落。却下。
  - 6/17 を call_count だけ単独 entry 作成 → 売上・コスト空の不完全 entry になる。現場通常入力に委ねる。却下。
